...
<script lang="ts">
	export let href: PlaylistLinkProps['href'];
</script>

<a class="grid grid-cols-[min-content_auto_auto] gap-4 p-3 group" {...$$props} {href}>
	<slot />
	<div
		class={`
    pointer
  ...
  `}
	>
		<svg viewBox="0 0 24 24" class="w-8 self-center z-[2]">
			<title />
			<g>
